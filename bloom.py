...
def bloom_sequential(model, dataloader, dev, means=None, stds=None):
    ...
    for i in range(len(layers)):
        ...
        for name in gpts:
            print(i, name)
            print('pruning ...')
            gpts[name].fasterprune(args.sparsity, prunen=args.prunen, prunem=args.prunem, percdamp=args.percdamp)
        for j in range(args.nsamples):
            outs[j] = layer(inps[j].unsqueeze(0), attention_mask=attention_mask, alibi=alibi)[0]
...
"""
@torch.no_grad()
def bloom_eval(model, test_encoding, dev, dataset: str, log_wandb: bool = False):
    print('Evaluation...')

    test_encoding = test_encoding.input_ids
    nsamples = test_encoding.numel() // model.seqlen
    ...
    for i in range(nsamples):
        batch = test_encoding[:, (i * model.seqlen):((i + 1) * model.seqlen)].to(dev)
        try:
            model(batch)
        except ValueError:
            pass
    ...
    test_encoding = test_encoding.to(dev)
    nlls = []
    for i in range(nsamples):
        ...
        shift_labels = test_encoding[:, (i * model.seqlen):((i + 1) * model.seqlen)][:, 1:]
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--percdamp', type=float, default=.01, help='Percent of the average Hessian diagonal to use for dampening.')
    ...
    train_dataloader, _ = get_loaders(args.dataset, nsamples=args.nsamples, seed=args.seed, model=args.model, seqlen=model.seqlen)

    if (args.sparsity or args.prunen) and not args.gmp:
        tick = time.time()
        bloom_sequential(model, train_dataloader, DEV)
    ...
    for dataset in ['wikitext2', 'ptb', 'c4']:
        _, test_encoding = get_loaders(dataset, seed=args.seed, model=args.model, seqlen=model.seqlen)
        print("Dataset:", dataset)
        bloom_eval(model, test_encoding, DEV, dataset, args.log_wandb)
