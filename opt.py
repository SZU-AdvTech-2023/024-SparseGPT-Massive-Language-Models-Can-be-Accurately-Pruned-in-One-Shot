...
"""
@torch.no_grad()
def opt_eval(model, test_encoding, dev, dataset: str, log_wandb: bool = False):
    print('Evaluating ...')
    ...
    for i in range(len(layers)):
        ...
        if args.gmp:
            ...
            for name in subset:
                ...
                threshold = torch.sort(torch.abs(W.flatten()))[0][int(W.numel() * args.sparsity)]
                W.data[torch.abs(W.data) <= threshold] = 0
...
if __name__ == '__main__':
    ...
    if (args.sparsity or args.prunen) and not args.gmp:
        tick = time.time()
        opt_sequential(model, train_dataloader, DEV)
        for name, param in model.named_parameters():
            print(name, torch.mean((param == 0).float()))
            if 'fc2' in name:
                break
    ...
    for dataset in ['wikitext2', 'ptb', 'c4']:
        ...
        print(dataset)
        opt_eval(model, test_encoding, DEV, dataset, args.log_wandb)
